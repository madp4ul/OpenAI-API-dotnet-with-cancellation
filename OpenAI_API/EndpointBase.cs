...
namespace OpenAI_API
{
    ...
    {
        private const string _userAgent = "okgodoit/dotnet_openai_api";
        ...
        private async Task<T> HttpRequest<T>(string url = null, HttpMethod verb = null, object postData = null) where T : ApiResultBase
        {
            ...
            {
                Debug.Print($"Issue parsing metadata of OpenAi Response.  Url: {url}, Error: {e}, Response: {resultAsString}.  This is probably ignorable.");
            }
		...
		*/
        ...
        /// <returns>The HttpResponseMessage of the response, which is confirmed to be successful.</returns>
        /// <exception cref="HttpRequestException">Throws an exception if a non-success HTTP response was returned</exception>
        protected async IAsyncEnumerable<T> HttpStreamingRequest<T>(string url = null, HttpMethod verb = null, object postData = null, [EnumeratorCancellation] CancellationToken cancellationToken = default) where T : ApiResultBase
        {
            var response = await HttpRequestRaw(url, verb, postData, true, cancellationToken);
